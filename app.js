...
app.get('/',(req, res)=>{
  new Promise((resolve,rej)=>{
    ...
    return new Promise((resolve,rej)=>{
      db.all(
        "SELECT * FROM data where time like '% %:00' order by rowid desc limit 196;"
        ,(err, rows2)=>{
        resolve(rows2);
      });
    }).then((rows2) => {
      ...
      res.render('index',{data : {aqi: aqi_id[0],
        iconid: aqi_id[1]}, curdata,hourdatas:{pm10:pm10,pm25:pm25,time:time},weakdata : {days:days,aqis:aqis}});
    })
  }).catch((err) => {
    console.log(err);
  });
